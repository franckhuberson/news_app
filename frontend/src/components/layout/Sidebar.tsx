            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${active 
                    ? 'text-primary-500 bg-gray-50 dark:bg-gray-800/50 font-bold' 
      ...
      <nav className="flex-1 overflow-y-auto">
        ...
        <div
          ...
          className={`
            ...
            ${isActive('/admin') 
              ? 'border-primary-500 bg-gray-50 dark:bg-gray-800/50 text-primary-500' 
              : 'border-transparent text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/30 hover:border-gray-300'
            }
            ${collapsed ? 'justify-center px-0' : 'px-6'}
          `}
        >
          <LayoutDashboard className={`w-5 h-5 ${isActive('/admin') ? 'text-primary-500' : ''}`} />
          {!collapsed && (
            <span className={`text-[11px] font-black uppercase tracking-wider ${isActive('/admin') ? 'text-primary-500' : ''}`}>
              TABLEAU DE BORD
            </span>
          )}
          {collapsed && (
        ...
        <div
          ...
          `}
        >
          <Calendar className={`w-5 h-5 ${isActive('/admin/articles/scheduled') ? 'text-primary-500' : ''}`} />
          {!collapsed && (
            <span className={`text-[11px] font-black uppercase tracking-wider ${isActive('/admin/articles/scheduled') ? 'text-primary-500' : ''}`}>
              ARTICLES PROGRAMMÉS
            </span>
          )}
          {collapsed && (
        ...
        {/* RÉDIGER UN ARTICLE - Placé après ARTICLES PROGRAMMÉS */}
        <div
          onClick={() => handleNavigation('/admin/articles/write')}
          className={`
            group relative flex items-center gap-4 py-4 mx-4 my-1 cursor-pointer
            transition-all duration-200 border-l-4
            ${isActive('/admin/articles/write') 
              ? 'border-primary-500 bg-linear-to-r from-orange-50 to-transparent dark:from-orange-950/20 text-primary-500' 
              : 'border-transparent text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/30 hover:border-gray-300'
            }
            ${collapsed ? 'justify-center px-0' : 'px-6'}
          `}
        >
          <PenTool className={`w-5 h-5 ${isActive('/admin/articles/write') ? 'text-primary-500' : ''}`} />
          {!collapsed && (
            <span className={`text-[11px] font-black uppercase tracking-wider ${isActive('/admin/articles/write') ? 'text-primary-500' : ''}`}>
              RÉDIGER UN ARTICLE
            </span>
          )}
          {collapsed && (
        ...
        <div
          ...
          `}
        >
          <TrendingUp className={`w-5 h-5 ${isActive('/admin/stats') ? 'text-primary-500' : ''}`} />
          {!collapsed && (
            <span className={`text-[11px] font-black uppercase tracking-wider ${isActive('/admin/stats') ? 'text-primary-500' : ''}`}>
              STATISTIQUES
            </span>
          )}
          {collapsed && (
        ...
        <div
          ...
          `}
        >
          <Settings className={`w-5 h-5 ${isActive('/admin/settings') ? 'text-primary-500' : ''}`} />
          {!collapsed && (
            <span className={`text-[11px] font-black uppercase tracking-wider ${isActive('/admin/settings') ? 'text-primary-500' : ''}`}>
              PARAMÈTRES
            </span>
          )}
          {collapsed && (
