...
export const Home: React.FC = () => {
  ...
  // ✅ Séparer les catégories en deux colonnes pour le footer
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-950 dark:to-gray-900 italic">
        ...
        <div className="container mx-auto px-4 py-3">
          <div className="flex flex-col md:flex-row md:items-center md:justify-center gap-4 md:gap-0">
            ...
            <div className="flex items-center justify-between w-full md:w-auto">
              <Link to="/" className="md:absolute md:left-4">
                <img 
                  src={logo} 
                  alt="AxioNews" 
          ...
          {isMobile && mobileMenuOpen && (
            <nav className="md:hidden mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
              <div className="flex flex-col gap-2">
                <button
                  onClick={() => filterArticlesByCategory('all')}
                  className={`text-left px-4 py-3 text-sm font-bold uppercase tracking-widest transition-colors rounded-lg ${
                    activeCategory === 'all' 
                      ? 'text-[#FF4500] bg-gray-50 dark:bg-gray-800' 
      ...
      <main className="container mx-auto py-4">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 border border-dashed border-gray-300 dark:border-gray-700 m-4">
            ...
            <button onClick={() => filterArticlesByCategory('all')} className="mt-4 text-[#FF4500] hover:text-[#E03D00] uppercase text-sm font-bold">
            ...
            {filteredArticles.map((article) => (
              <article key={article._id} className="group bg-white dark:bg-gray-900 border-r border-b border-gray-200 dark:border-gray-800 flex flex-col hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors duration-300">
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <h3 className="text-xl font-black uppercase leading-tight mb-4 group-hover:text-[#FF4500] transition-colors line-clamp-2 dark:text-white">{article.title}</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-3 leading-relaxed mb-6">{article.summary}</p>
                  <div className="mt-auto pt-4 border-t border-gray-100 dark:border-gray-800">
                    <Link to={`/article/${article._id}`} className="text-[11px] font-black uppercase tracking-[0.2em] group-hover:translate-x-2 transition-transform inline-flex items-center gap-2 dark:text-white hover:text-[#FF4500]">
      ...
      {/* Footer avec catégories sur 2 colonnes */}
      <footer className="bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 mt-16">
        <div className="container mx-auto px-4 py-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
            ...
            {/* ✅ Section Catégories en 2 colonnes */}
            <div>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                {categories.map((cat) => (
                  <button
                    ...
                    className="text-sm text-gray-600 dark:text-gray-400 hover:text-[#FF4500] transition-colors text-left"
                  >
                    {cat}
                  </button>
                ))}
            ...
            <div>
              ...
              <form onSubmit={handleSubscribe} className="flex flex-col gap-3">
                ...
                <input type="email" placeholder="Votre email" value={email} onChange={(e) => setEmail(e.target.value)} required className="px-4 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-[#FF4500] text-sm" />
                <button type="submit" disabled={subscribing} className="px-4 py-2 bg-[#FF4500] text-white text-sm rounded-lg hover:bg-[#E03D00] transition-colors disabled:opacity-50">
          ...
          <div className="border-t border-gray-200 dark:border-gray-800 pt-4 text-center">
            <p className="text-xs text-gray-500 dark:text-gray-400">© {new Date().getFullYear()} Axio News. Tous droits réservés.</p>
          </div>
        </div>
      </footer>
    </div>
