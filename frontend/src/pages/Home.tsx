...
export const Home: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-950 dark:to-gray-900">
      {/* HEADER - Version du code qui fonctionne */}
      ...
      <main className="container mx-auto py-8">
        ...
        {/* Section Une - Version du code qui fonctionne */}
              ...
              <div className="flex flex-col bg-gradient-to-b from-gray-50 to-white dark:from-gray-800/50 dark:to-gray-900">
                <div className="flex-1">
                  <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                    {[1, 2, 3, 4].map((item) => (
                      <li key={item} className="p-5 hover:bg-white dark:hover:bg-gray-800 transition-all duration-300 cursor-pointer group relative overflow-hidden">
                        ...
                        <h3 className="font-bold text-lg leading-tight group-hover:text-[#FF4500] dark:text-white transition-colors pl-3">
                          {item === 1 && "Marchés boursiers : forte hausse portée par la tech"}
                          {item === 2 && "Élections locales : les premiers résultats sont tombés"}
                          {item === 3 && "Intelligence Artificielle : les nouvelles percées"}
                          {item === 4 && "Climat : accord historique à la COP30"}
                        </h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 pl-3">
                          {item === 1 && "Les indices européens atteignent des sommets inédits."}
                          {item === 2 && "Taux de participation historique dans plusieurs régions."}
                          {item === 3 && "Des avancées majeures dans le domaine de la santé."}
                          {item === 4 && "Les nations s'engagent pour la neutralité carbone."}
                        </p>
                      </li>
                    ))}
                  </ul>
        ...
        {/* Grille d'articles - Version du code qui fonctionne */}
        {loading ? (
