...
export const ArticleDetail: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 flex items-center justify-center italic">
            ...
            <nav className="flex items-center justify-center md:justify-start gap-4 md:gap-6 border-t md:border-none border-gray-100 dark:border-gray-800 pt-3 md:pt-0">
              {['Politique', 'Santé', 'Tech', 'Économie', 'Culture', 'Sports'].map((theme) => (
                <Link
                  ...
                  className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-gray-600 dark:text-gray-400 hover:text-primary-500 transition-colors"
                >
                  {theme}
                </Link>
              ))}
          ...
          {article.imageUrl && (
            <div className="relative w-full h-72 md:h-96 overflow-hidden">
              ...
              <div className="absolute bottom-4 left-4 flex items-center gap-2">
                <span className="bg-primary-500 text-white px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider shadow-lg">
                  Amaya News
                </span>
            ...
            <div>
              ...
              <ul className="space-y-2">
                {['Politique', 'Santé', 'Tech', 'Économie', 'Culture', 'Sport'].map((cat) => (
                  <li key={cat}>
                    <Link to={`/${cat.toLowerCase()}`} className="text-sm text-gray-600 dark:text-gray-400 hover:text-primary-500 transition-colors">
                      {cat}
                    </Link>
                  </li>
                ))}
