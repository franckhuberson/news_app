...
import { Calendar, Clock, Trash2, Edit3, Newspaper, Eye } from 'lucide-react';
...
export const ScheduledArticles: React.FC = () => {
  ...
  const handleCancelSchedule = async (id: string) => {
    ...
    try {
      const response = await api.delete(`/articles/${id}/schedule`);
  ...
  // ✅ Rediriger vers WriteArticle avec l'article complet
  const handleEdit = (article: Article) => {
    navigate('/admin/articles/write', { state: { article } });
  };
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b-4 border-black dark:border-white">
            <div>
              ...
              <h1 className="text-4xl font-black uppercase tracking-tighter dark:text-white italic">
                Publications <span className="text-primary-500">programmées</span>
              </h1>
            </div>
          </div>
                ...
                {articles.map((article) => (
                  <article key={article._id} className="group flex flex-col bg-white dark:bg-gray-900 border-r border-b border-black dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-all duration-300">
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                      ...
                      {/* ✅ TROIS BOUTONS : Lire + Modifier + Annuler */}
                      <div className="mt-auto pt-6 border-t border-gray-100 dark:border-gray-800 flex gap-2">
                        <button 
                          onClick={() => handleReadArticle(article)}
                          className="flex-1 py-3 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-md flex items-center justify-center gap-2"
                        >
                          <Eye size={14} />
                          Lire
                        </button>
